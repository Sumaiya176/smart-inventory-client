...
// export default function CreateProductModal({ onClose, onSuccess, productToUpdate }: CreateProductModalProps) {
...
//               <input
//                 type="text"
//                 defaultValue={productToUpdate?.name || formData.name}
//                 //value={formData.name}
...
export default function CreateProductModal({ onClose, onSuccess, productToUpdate }: CreateProductModalProps) {
  ...
  const { categories } = useAppSelector((state) => state.categories);
  const [loading, setLoading] = useState(false);
  const isEditMode = !!productToUpdate;
  ...
  useEffect(() => {
    if (productToUpdate) {
      setFormData({
        ...
        stockQuantity: productToUpdate.stockQuantity?.toString() || '',
        minimumStockThreshold: productToUpdate.minimumStockThreshold?.toString() || '5',
            ...
            <div>
              ...
              <div className="flex space-x-2">
                <input
                  ...
                  placeholder="Product SKU"
                  disabled={isEditMode} // Disable SKU editing in edit mode
                />
                {!isEditMode && (
                  <button
                    type="button"
